        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 p-1 bg-muted/60 border border-border rounded-lg">
            <Link href="/community?sort=latest">
              <button className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-md transition-all cursor-pointer ${sortBy === 'latest' ? 'bg-card text-foreground shadow-xs' : 'text-muted-foreground hover:text-foreground'}`}>
                Latest
              </button>
            </Link>
            <Link href="/community?sort=top">
              <button className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-md transition-all cursor-pointer ${sortBy === 'top' ? 'bg-card text-foreground shadow-xs' : 'text-muted-foreground hover:text-foreground'}`}>
      ...
      {!user && (
        <div className="mb-6 flex items-center gap-3 px-4 py-3 rounded-xl border border-violet-500/30 bg-violet-500/10 dark:bg-violet-950/20 text-sm">
          ...
          <p className="text-muted-foreground">
            ...
            <Link href="/register" className="text-violet-600 dark:text-violet-400 font-semibold hover:underline transition-colors">
          ...
          {publicProblems.map((p) => (
            ...
            >
              ...
              <CardContent className="p-6 flex-grow flex flex-col justify-between">
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {p.tags && p.tags.length > 0 ? (
                      p.tags.map((tag, i) => (
                        <Badge key={i} variant="secondary" className="bg-muted border border-border text-muted-foreground text-[9px] px-2">
                          {tag}
                        </Badge>
                      ))
                    ) : (
                      <Badge variant="outline" className="text-muted-foreground border-border text-[9px] px-2">General</Badge>
                    )}
                  </div>
                ...
                {/* Meta details */}
                <div className="border-t border-border/60 pt-4 mt-6 space-y-2">
                  {/* Author */}
                  <div className="flex items-center text-[10px] text-muted-foreground gap-1.5">
                    <User className="h-3 w-3 text-muted-foreground" />
                    <span className="truncate">
                      By: <span className="font-semibold text-foreground">{p.authorName || p.authorEmail || 'Anonymous'}</span>
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-[10px] text-muted-foreground font-mono">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex items-center gap-1 text-violet-600 dark:text-violet-400 font-semibold bg-violet-500/10 px-2 py-0.5 rounded border border-violet-500/20">
